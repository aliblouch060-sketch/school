...
const TABLES = [
  ...
  {
    name: 'attendance',
    columns: ['id', 'student_id', 'attendance_date', 'status', 'remarks', 'created_at'],
  ...
  { name: 'timetable', columns: ['id', 'class_name', 'section', 'weekday', 'period_name', 'subject', 'teacher_name', 'start_time', 'end_time', 'created_at'] },
  {
    ...
    insertColumns: ['username', 'password_hash', 'role', 'created_at'],
...
async function copyTable(sqliteDb, pgClient, table) {
  ...
  const conflictTarget = table.conflictTarget || ['id'];
  const updateColumns = insertColumns
    .filter((column) => column !== 'id' && !conflictTarget.includes(column))
  ...
  const sql = `
    ...
    VALUES (${placeholders})
    ON CONFLICT (${conflictTarget.join(', ')}) DO UPDATE SET ${updateColumns}
  `;
