...
const attendanceBulkForm = document.getElementById('attendanceBulkForm');
...
const attendanceBulkClass = document.getElementById('attendanceBulkClass');
...
const attendanceSummaryClassFilter = document.getElementById('attendanceSummaryClassFilter');
const loadAttendanceBtn = document.getElementById('loadAttendanceBtn');
const loadAttendanceSummaryBtn = document.getElementById('loadAttendanceSummaryBtn');
const saveAttendanceSummaryBtn = document.getElementById('saveAttendanceSummaryBtn');
const printAttendanceSummaryBtn = document.getElementById('printAttendanceSummaryBtn');
...
const attendanceClassSummaryTableBody = document.querySelector('#attendanceClassSummaryTable tbody');
const attendanceStudentSummaryTableBody = document.querySelector('#attendanceStudentSummaryTable tbody');
...
const attendancePrintArea = document.createElement('div');
...
const attendanceSummaryMeta = document.getElementById('attendanceSummaryMeta');
const i18nElements = {
...
function setupControls() {
  ...
  fillClassSelect(attendanceBulkClass);
  ...
  fillClassSelect(attendanceSummaryClassFilter, 'All Classes (Monthly Summary)');
  ...
  if (attendanceBulkForm?.elements.attendanceDate && !attendanceBulkForm.elements.attendanceDate.value) {
    attendanceBulkForm.elements.attendanceDate.value = new Date().toISOString().slice(0, 10);
  }
  ...
  if (feeForm?.elements.feeMonth && !feeForm.elements.feeMonth.value) {
    feeForm.elements.feeMonth.value = currentMonth;
  }
  ...
  if (!attendancePrintArea.isConnected) {
    attendancePrintArea.className = 'print-area attendance-print-area';
    attendancePrintArea.setAttribute('aria-hidden', 'true');
    document.body.appendChild(attendancePrintArea);
  }
...
function renderAttendance(rows) {
  ...
  if (!rows.length) {
    const tr = document.createElement('tr');
    tr.innerHTML = '<td colspan="6">Is date ya class ke liye abhi attendance record nahi mila.</td>';
  ...
  rows.slice(0, 40).forEach((item) => {
    ...
    const actionCell =
      !item.id && canEditEntity('attendance')
        ? `<td class="action-cell"><button type="button" class="action-btn" data-row="${encodeURIComponent(JSON.stringify(item || {}))}" onclick="handleAttendanceQuickMark(event, this)">Mark Now</button></td>`
...
function formatAttendanceMonthLabel(monthKey) {
...
function renderAttendanceMonthlySummary(summary) {
  currentAttendanceSummary = summary || null;
  if (saveAttendanceSummaryBtn) {
    saveAttendanceSummaryBtn.disabled = !summary;
  }
  if (printAttendanceSummaryBtn) {
    printAttendanceSummaryBtn.disabled = !summary;
  }

  if (attendanceSummaryMeta) {
    if (!summary) {
      attendanceSummaryMeta.textContent = '';
    } else {
      const scope = summary.className ? `Class: ${summary.className}` : 'All Classes';
      attendanceSummaryMeta.textContent =
        `${formatAttendanceMonthLabel(summary.month)} | ${scope} | Working days are based on dates marked in attendance. ` +
        `Attendance % = Present + Late / working days after student admission date.`;
    }
  }

  if (attendanceClassSummaryTableBody) {
    attendanceClassSummaryTableBody.innerHTML = '';
    const classRows = Array.isArray(summary?.classSummaries) ? summary.classSummaries : [];
    if (!classRows.length) {
      const tr = document.createElement('tr');
      tr.innerHTML = '<td colspan="8">Is month ke liye class summary available nahi hai.</td>';
      attendanceClassSummaryTableBody.appendChild(tr);
    } else {
      classRows.forEach((item) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          <td>${escapeHtml(item.className || '-')}</td>
          <td>${Number(item.totalStudents || 0)}</td>
          <td>${Number(item.workingDays || 0)}</td>
          <td>${Number(item.markedDays || 0)}</td>
          <td>${Number(item.absentDays || 0)}</td>
          <td>${Number(item.lateDays || 0)}</td>
          <td>${Number(item.notMarkedDays || 0)}</td>
          <td>${Number(item.attendancePercentage || 0).toFixed(2)}%</td>
        `;
        attendanceClassSummaryTableBody.appendChild(tr);
      });
    }
  ...
  if (attendanceStudentSummaryTableBody) {
    attendanceStudentSummaryTableBody.innerHTML = '';
    const studentRows = Array.isArray(summary?.studentSummaries) ? summary.studentSummaries : [];
    if (!studentRows.length) {
      const tr = document.createElement('tr');
      tr.innerHTML = '<td colspan="9">Is month ke liye student summary available nahi hai.</td>';
      attendanceStudentSummaryTableBody.appendChild(tr);
    } else {
      studentRows.slice(0, 300).forEach((item) => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
          ...
          <td>${Number(item.workingDays || 0)}</td>
          <td>${Number(item.presentDays || 0)}</td>
          <td>${Number(item.absentDays || 0)}</td>
          <td>${Number(item.lateDays || 0)}</td>
          <td>${Number(item.notMarkedDays || 0)}</td>
          <td>${Number(item.attendancePercentage || 0).toFixed(2)}%</td>
...
function buildAttendanceSummaryDocument(summary) {
  const classRows = (summary.classSummaries || [])
    .map(
      (item) => `
        <tr>
          <td>${escapeHtml(item.className || '-')}</td>
  ...
  const studentRows = (summary.studentSummaries || [])
    .map(
      (item) => `
        <tr>
          <td>${escapeHtml(item.fullName || '-')}</td>
...
<head>
  ...
  <style>
    ...
    .report-shell { max-width: 1200px; margin: 0 auto; background: #fff; border: 1px solid #c9d8d0; border-radius: 18px; padding: 24px; }
    .report-head { display: flex; justify-content: space-between; gap: 16px; align-items: center; border-bottom: 2px solid #d6e2dc; padding-bottom: 14px; margin-bottom: 18px; }
    .report-brand { display: flex; align-items: center; gap: 14px; }
    .report-brand img { width: 58px; height: 58px; object-fit: contain; }
    .report-brand h1, .report-brand p, .report-meta p { margin: 0; }
    .report-brand h1 { font-size: 26px; color: #0f5343; text-transform: uppercase; }
    .report-meta p + p { margin-top: 6px; }
    .summary-note { margin: 0 0 18px; color: #305246; }
    h2 { margin: 24px 0 10px; color: #173f34; }
  ...
  <div class="report-shell">
    <div class="report-head">
      ...
      <div class="report-meta">
        <p><strong>Month:</strong> ${escapeHtml(formatAttendanceMonthLabel(summary.month))}</p>
        <p><strong>Scope:</strong> ${escapeHtml(summary.className || 'All Classes')}</p>
        <p><strong>Generated:</strong> ${escapeHtml(new Date().toLocaleString())}</p>
      </div>
    </div>
    <p class="summary-note">Attendance % = Present + Late / working days after admission date. Working days are based on dates marked in the attendance system for the selected month.</p>
    <h2>Class Summary</h2>
    <table>
      <thead>
        <tr>
      ...
      <tbody>${classRows || '<tr><td colspan="8">No class summary available.</td></tr>'}</tbody>
    </table>
    ...
    <table>
      ...
      <tbody>${studentRows || '<tr><td colspan="9">No student summary available.</td></tr>'}</tbody>
    </table>
  </div>
</body>
</html>`;
...
async function loadAttendanceMonthlySummary() {
  const month = attendanceMonthFilter?.value || new Date().toISOString().slice(0, 7);
  const selectedClass = attendanceSummaryClassFilter?.value || '';
  const params = new URLSearchParams({ month });
  if (selectedClass) params.set('className', selectedClass);
  const summary = await api(`/api/attendance/monthly-summary?${params.toString()}`);
  renderAttendanceMonthlySummary(summary);
  return summary;
}

async function saveAttendanceMonthlySummaryFile() {
  if (!currentAttendanceSummary) {
    throw new Error('Pehle monthly summary generate karein.');
  }

  const html = buildAttendanceSummaryDocument(currentAttendanceSummary);
  const response = await api('/api/attendance/monthly-summary/save', {
    method: 'POST',
    body: JSON.stringify({
      month: currentAttendanceSummary.month,
      className: currentAttendanceSummary.className,
      html,
    }),
  });
  setMessage('attendanceSummaryMsg', `${response.message} Path: ${response.filePath}`);
}

function printAttendanceMonthlySummary() {
  if (!currentAttendanceSummary) {
    throw new Error('Pehle monthly summary generate karein.');
  }

  const title = `attendance_summary_${currentAttendanceSummary.className || 'all_classes'}_${currentAttendanceSummary.month}`;
  const html = buildAttendanceSummaryDocument(currentAttendanceSummary);
  printHtmlContent(html, title, { area: attendancePrintArea, messageId: 'attendanceSummaryMsg' });
  setMessage('attendanceSummaryMsg', 'Print dialog khul gaya hai. Save as PDF select karein.');
}

async function submitBulkAttendance() {
  const payload = getFormData(attendanceBulkForm);
  const className = String(payload.className || '').trim();
  const attendanceDate = String(payload.attendanceDate || '').trim();
  const status = String(payload.status || '').trim();
  const remarks = String(payload.remarks || '').trim();

  if (!className || !attendanceDate || !status) {
    throw new Error('Class, date aur status required hain.');
  }

  const students = getStudentsByClass(className);
  if (!students.length) {
    throw new Error('Is class mein koi student nahi mila.');
  }
  ...
  const response = await api('/api/attendance/bulk', {
    method: 'POST',
    body: JSON.stringify({ attendanceDate, entries }),
  });

  attendanceClassFilter.value = className;
  attendanceDateFilter.value = attendanceDate;
  if (attendanceMonthFilter) {
    attendanceMonthFilter.value = attendanceDate.slice(0, 7);
  }
  if (attendanceSummaryClassFilter) {
    attendanceSummaryClassFilter.value = className;
  }

  setMessage('attendanceMsg', `${response.savedCount || entries.length} students ki attendance save ho gayi.`);
  await Promise.all([loadAttendanceRecords(), loadAttendanceMonthlySummary(), loadDashboard()]);
  return response;
}
...
function prefillAttendanceForm(row) {
  if (!row) return;
  clearEditMode('attendance');
  setFormValue(attendanceForm, 'className', row.class_name || '');
  populateStudentSelects();
  setFormValue(attendanceForm, 'studentId', row.student_id || '');
  setFormValue(attendanceForm, 'attendanceDate', row.attendance_date || new Date().toISOString().slice(0, 10));
  setFormValue(attendanceForm, 'status', row.status && row.status !== 'Not Marked' ? row.status : 'Present');
  setFormValue(attendanceForm, 'remarks', row.remarks || '');
  scrollFormIntoView(attendanceForm);
}
...
async function refreshData() {
  ...
  await Promise.all([loadAttendanceRecords(), loadAttendanceMonthlySummary(), loadResultRecords(), loadDashboard()]);
...
attendanceBulkForm?.addEventListener('submit', async (e) => {
  e.preventDefault();
  try {
...
saveAttendanceSummaryBtn?.addEventListener('click', () => {
  saveAttendanceMonthlySummaryFile().catch((error) => setMessage('attendanceSummaryMsg', error.message, 'error'));
});
printAttendanceSummaryBtn?.addEventListener('click', () => {
  try {
    printAttendanceMonthlySummary();
  } catch (error) {
    setMessage('attendanceSummaryMsg', error.message, 'error');
  }
});
...
attendanceMonthFilter?.addEventListener('change', () => loadAttendanceMonthlySummary().catch((error) => setMessage('attendanceSummaryMsg', error.message, 'error')));
attendanceSummaryClassFilter?.addEventListener('change', () => loadAttendanceMonthlySummary().catch((error) => setMessage('attendanceSummaryMsg', error.message, 'error')));
loadAttendanceSummaryBtn?.addEventListener('click', () => loadAttendanceMonthlySummary().catch((error) => setMessage('attendanceSummaryMsg', error.message, 'error')));
...
function handleAttendanceQuickMark(event, button) {
  if (event) {
    event.preventDefault();
    event.stopPropagation();
  }

  if (!canEditEntity('attendance')) {
    setMessage('attendanceMsg', 'Attendance mark karne ki permission available nahi hai.', 'error');
    return;
  }

  const row = decodeRowPayload(button);
  if (!row) {
    setMessage('attendanceMsg', 'Student data load nahi hui.', 'error');
    return;
  }

  prefillAttendanceForm(row);
  setMessage('attendanceMsg', `${row.full_name || 'Student'} ke liye form ready hai. Status select karke save karein.`);
}
